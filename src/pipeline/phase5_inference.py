...
def run_inference(config_path="configs/model_params.yaml", data_dir="data"):
    print("\n[Phase 5] Test predictions and submission...")
    ...
    print(f"  Predicting {n_targets} models with classifier chains...")
    ...
    np.save(f"{data_dir}/test_predictions_day2.npy", test_preds)
    ...
    submit.write_parquet(f"{data_dir}/submit_day2.parquet")
    print(f"  Submit saved to {data_dir}/submit_day2.parquet")

    del test_preds
    gc.collect()

    return {"submit_path": f"{data_dir}/submit_day2.parquet"}
